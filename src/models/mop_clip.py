...
class MixturePromptCLIP(nn.Module):
    ...
    NOW UPDATED WITH *PER-CLASS PROMPT OFFSETS*:
        prompt[c, k] = base_text_features[c] + prompt_offsets[c, k]

    This is the critical fix that allows the model to learn class-specific
    representations and escape the ~3% accuracy plateau.
    """
    ...
    def __init__(
        ...
        self.D = self.clip.text_projection.shape[0]  # 512 for ViT-B/32
        ...
        text_feats = []
        with torch.no_grad():
            for i in tqdm(range(0, tokenized.size(0), batch_size),
                          desc="Encoding text prompts", ncols=80):
        ...
        # Keep text features on CPU permanently
        ...
        # Move encoder back to CPU before model.to(device)
        self.clip.to("cpu")

        # -------------------------------------------------------------
        # *** FIXED: Per-class learnable prompt offsets ***
        #
        # Shape: (C, K, D)
        # Instead of global (K, D), each class now has its own offsets.
        # This is crucial for classification performance.
        # -------------------------------------------------------------
    ...
    # ==========================================================
    # Force base_text_features to remain on CPU
    # ==========================================================
    def _apply(self, fn):
    ...
    def _batch_prompt_features(self, labels: torch.Tensor, device: torch.device):
        ...
        # 2. Per-class prompt offsets (already on GPU after model.to(device))
        #    labels are also on GPU → perfect match
    ...
    def predict(self, images):
        ...
        final_scores = torch.cat(cpu_accum, dim=1).to(device)
