"""
Visualization script for Mixture-of-Prompts CLIP model - Oxford-IIIT Pet dataset.
...
def main():
    ...
    dataset_type = config["dataset"].get("type", "oxford_pets")

    if dataset_type != "oxford_pets":
        raise ValueError(f"Expected dataset type 'oxford_pets', got '{dataset_type}'")
    ...
    print(f"Target class index: {class_idx} (pet='{cls_name}')")
    ...
    train_dataset = get_oxford_pets(root, "train", use_torchvision=True)
    val_dataset = get_oxford_pets(root, "val", use_torchvision=True)
    ...
    raw_train_dataset = get_oxford_pets(root, "train", transform=raw_transform, use_torchvision=True)
    raw_val_dataset = get_oxford_pets(root, "val", transform=raw_transform, use_torchvision=True)
