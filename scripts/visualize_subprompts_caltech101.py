"""
Visualization script for Mixture-of-Prompts CLIP model - Caltech101 dataset.
...
def main():
    ...
    dataset_type = config["dataset"].get("type", "caltech101")

    if dataset_type != "caltech101":
        raise ValueError(f"Expected dataset type 'caltech101', got '{dataset_type}'")
    ...
    print(f"Target class index: {class_idx} (object='{cls_name}')")
    ...
    train_dataset = get_caltech101(root, "train", use_torchvision=True)
    val_dataset = get_caltech101(root, "val", use_torchvision=True)
    ...
    raw_train_dataset = get_caltech101(root, "train", transform=raw_transform, use_torchvision=True)
    raw_val_dataset = get_caltech101(root, "val", transform=raw_transform, use_torchvision=True)
