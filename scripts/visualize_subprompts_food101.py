"""
Visualization script for Mixture-of-Prompts CLIP model - Food-101 dataset.
...
def main():
    ...
    dataset_type = config["dataset"].get("type", "food101")

    if dataset_type != "food101":
        raise ValueError(f"Expected dataset type 'food101', got '{dataset_type}'")
    ...
    print(f"Target class index: {class_idx} (food='{cls_name}')")
    ...
    train_dataset = get_food101(root, "train", use_torchvision=True)
    val_dataset = get_food101(root, "val", use_torchvision=True)
    ...
    raw_train_dataset = get_food101(root, "train", transform=raw_transform, use_torchvision=True)
    raw_val_dataset = get_food101(root, "val", transform=raw_transform, use_torchvision=True)
