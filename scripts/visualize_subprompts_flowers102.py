"""
Visualization script for Mixture-of-Prompts CLIP model - Flowers102 dataset.
...
def main():
    ...
    dataset_type = config["dataset"].get("type", "flowers102")

    if dataset_type != "flowers102":
        raise ValueError(f"Expected dataset type 'flowers102', got '{dataset_type}'")
    ...
    print(f"Target class index: {class_idx} (flower='{cls_name}')")
    ...
    train_dataset = get_flowers102(root, "train", use_torchvision=True)
    val_dataset = get_flowers102(root, "val", use_torchvision=True)
    ...
    raw_train_dataset = get_flowers102(root, "train", transform=raw_transform, use_torchvision=True)
    raw_val_dataset = get_flowers102(root, "val", transform=raw_transform, use_torchvision=True)
