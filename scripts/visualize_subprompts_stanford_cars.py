"""
Visualization script for Mixture-of-Prompts CLIP model - Stanford Cars dataset.
...
def find_class_index_by_name(metadata, name: str):
    """
    Find class index whose 'full_name' field matches the given name (case-insensitive).
    """
    name = name.lower()
    matches = []
    for idx, md in enumerate(metadata):
        full_name = md.get("full_name", md.get("species", "")).lower()
...
def main():
    ...
    dataset_type = config["dataset"].get("type", "stanford_cars")

    if dataset_type != "stanford_cars":
        raise ValueError(f"Expected dataset type 'stanford_cars', got '{dataset_type}'")
    ...
    if args.class_idx is not None:
        ...
        cls_name = metadata[class_idx].get("full_name", metadata[class_idx].get("species", f"class_{class_idx}"))
    ...
    print(f"Target class index: {class_idx} (car='{cls_name}')")
    ...
    train_dataset = get_stanford_cars(root, "train", use_torchvision=False)
    val_dataset = get_stanford_cars(root, "val", use_torchvision=False)
    ...
    raw_train_dataset = get_stanford_cars(root, "train", transform=raw_transform, use_torchvision=False)
    raw_val_dataset = get_stanford_cars(root, "val", transform=raw_transform, use_torchvision=False)
