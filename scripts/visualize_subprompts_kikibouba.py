"""
Visualization script for Mixture-of-Prompts CLIP model - KikiBouba dataset.
...
def find_class_index_by_name(metadata, name: str):
    ...
    for idx, md in enumerate(metadata):
        species = md.get("species", "").lower()
        if name in species or species == name:
            matches.append(idx)
    if not matches:
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/kikibouba_v1.yaml")
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--split", type=str, default="val", choices=["train", "val", "test"])
    parser.add_argument("--class_idx", type=int, default=None,
                        help="Target class index (0-based).")
    parser.add_argument("--class_name", type=str, default=None,
                        help="Alternatively, class name to search for.")
    parser.add_argument("--top_n", type=int, default=5,
                        help="Top-N images per sub-prompt to visualize.")
    parser.add_argument("--out_dir", type=str, default="subprompt_viz_kikibouba",
                        help="Where to save visualization grids.")
    parser.add_argument("--temperature", type=float, default=None,
                        help="Temperature for soft assignments (higher = softer).")
    ...
    dataset_type = config["dataset"].get("type", "kikibouba")
    version = config["dataset"].get("version", "v1")

    if dataset_type != "kikibouba":
        raise ValueError(f"Expected dataset type 'kikibouba', got '{dataset_type}'")
    ...
    if args.class_idx is not None:
        ...
        cls_name = metadata[class_idx].get("species", f"class_{class_idx}")
    ...
    print(f"Target class index: {class_idx} (class='{cls_name}')")

    # ---------------------------
    # Load dataset split(s) - combine train and val
    # ---------------------------
    from torch.utils.data import ConcatDataset
    
    train_dataset = get_kikibouba(root, "train", version=version)
    val_dataset = get_kikibouba(root, "val", version=version)
    ...
    raw_train_dataset = get_kikibouba(root, "train", transform=raw_transform, version=version)
    raw_val_dataset = get_kikibouba(root, "val", transform=raw_transform, version=version)
    ...
    # Determine K from checkpoint shape
    ...
    # ---------------------------
    all_images_per_k = {k: [] for k in range(K)}
    ...
    per_k = {}
    for k in range(K):
        all_images_per_k[k].sort(key=lambda x: x[0], reverse=True)
    ...
    for k in range(K):
        entries = per_k[k][:args.top_n]

        imgs = []
        scores = []
        for score, dataset_idx in entries:
            img_raw, _ = raw_dataset[dataset_idx]
    ...
    for k, imgs in all_row_imgs:
        ...
        for _ in range(num_missing):
            blank = torch.ones(3, 224, 224) * 0.3
            blank = add_gamma_text(blank, -1.0, font_size=30)
    ...
    safe_cls_name = cls_name.replace(' ', '_').replace('/', '_').replace('\\', '_')
