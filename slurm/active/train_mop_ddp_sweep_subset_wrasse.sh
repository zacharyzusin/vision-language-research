...
#SBATCH --job-name=inat_wrasse_sweep # Job name
...
#SBATCH --output=logs/sweep_wrasse_1gpu_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_wrasse_1gpu_%j.err     # Error file
...
echo "Running wandb sweep agent for iNaturalist Wrasse subset sweep ID: $SWEEP_ID"

# Single-GPU setup
echo "Single-GPU configuration (iNaturalist Wrasse subset):"
...
# The agent will call run_sweep_training_subset_wrasse.sh for each sweep run
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_subset_wrasse.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_subset_wrasse.sh

# Run wandb agent - it will execute run_sweep_training_subset_wrasse.sh for each sweep run
