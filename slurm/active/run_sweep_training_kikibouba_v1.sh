#!/bin/bash
# Wrapper script to launch multi-node DDP training for wandb sweep (KikiBouba v1)
...
LAUNCH_SCRIPT="${PWD}/launch_torchrun_sweep_kikibouba_v1_${SLURM_JOB_ID}.sh"
...
echo "Node $(hostname) (rank $NODE_RANK) launching torchrun for KikiBouba v1 sweep run..."
...
torchrun \
    ...
    train.py --config configs/kikibouba_v1.yaml
LAUNCH_EOF
