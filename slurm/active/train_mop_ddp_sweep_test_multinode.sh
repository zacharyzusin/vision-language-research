...
# Fail fast if iNaturalist dataset isn't present
if [ ! -d "data/iNat2021/2021/train" ] || [ ! -d "data/iNat2021/2021/val" ]; then
  echo "ERROR: iNaturalist dataset not found under data/iNat2021/2021/{train,val}"
...
echo "Running wandb sweep agent for test sweep ID: $SWEEP_ID"
...
echo "Multi-node DDP configuration (TEST):"
...
# The agent will call run_sweep_training.sh for each sweep run, which launches torchrun
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training.sh

# Run wandb agent - it will execute run_sweep_training.sh for each sweep run
# The wrapper script handles multi-node DDP setup
# By default, agent runs until sweep is complete (2 K values for test)
