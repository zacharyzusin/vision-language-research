...
#SBATCH --job-name=mop_flowers_k8    # Job name
...
#SBATCH --output=logs/train_flowers102_k8_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/train_flowers102_k8_%j.err     # Error file
...
echo "Config: configs/flowers102.yaml"
echo "GPUs: $NPROC"
echo ""
...
torchrun --standalone --nproc_per_node="${NPROC}" \
    train.py \
    --config configs/flowers102.yaml
