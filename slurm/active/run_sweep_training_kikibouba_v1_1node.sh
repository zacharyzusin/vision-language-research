#!/bin/bash
# Wrapper script to launch single-node DDP training for wandb sweep (KikiBouba v1)
...
echo "Launching torchrun for KikiBouba v1 sweep run..."
echo "  GPUs: $NPROC"

# Single-node: use standalone mode (simpler, no need for master address/port)
torchrun --standalone --nproc_per_node="${NPROC}" train.py --config configs/kikibouba_v1.yaml
