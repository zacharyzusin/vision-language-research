#!/bin/bash
# Script to initialize wandb sweep for KikiBouba v1 and submit Slurm jobs to run sweep agents
# Usage: ./launch_sweep_kikibouba_v1.sh [num_agents]
...
echo "=== Initializing W&B Sweep for KikiBouba v1 ==="
echo "Sweep config: sweep_k_values_kikibouba_v1.yaml"
echo "Number of agents: $NUM_AGENTS"
echo ""

# Initialize the sweep and get the sweep ID
SWEEP_ID=$(wandb sweep sweep_k_values_kikibouba_v1.yaml 2>&1 | grep -oP 'wandb agent [^\s]+' | awk '{print $3}' || echo "")
...
for i in $(seq 1 $NUM_AGENTS); do
    ...
    JOB_ID=$(sbatch \
        ...
        slurm/active/train_mop_ddp_sweep_kikibouba_v1.sh 2>&1 | grep -oP '\d+' | head -1)
...
echo ""
echo "View logs:"
echo "  tail -f logs/sweep_kb1_*.out"
