...
#SBATCH --job-name=kb2_sweep_1gpu    # Job name
...
#SBATCH --output=logs/sweep_kb2_1gpu_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_kb2_1gpu_%j.err     # Error file
...
# The agent will call run_sweep_training_kikibouba_v2_1gpu.sh for each sweep run
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_kikibouba_v2_1gpu.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_kikibouba_v2_1gpu.sh

# Run wandb agent - it will execute run_sweep_training_kikibouba_v2_1gpu.sh for each sweep run
