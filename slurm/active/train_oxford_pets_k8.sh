...
#SBATCH --job-name=mop_oxford_pets   # Job name
...
#SBATCH --output=logs/train_oxford_pets_k8_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/train_oxford_pets_k8_%j.err     # Error file
...
echo "Config: configs/oxford_pets.yaml"
echo "GPUs: $NPROC"
echo ""
...
torchrun --standalone --nproc_per_node="${NPROC}" \
    train.py \
    --config configs/oxford_pets.yaml
