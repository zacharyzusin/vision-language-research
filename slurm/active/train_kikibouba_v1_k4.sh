...
#SBATCH --job-name=kikibouba_k4      # Job name
#SBATCH --partition=short             # GPU partition
...
#SBATCH --gres=gpu:2                  # Request 2 GPUs (more flexible than 4)
#SBATCH --exclude=ins082              # ins082 has a bad GPU device handle
#SBATCH --output=logs/train_kikibouba_v1_k4_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/train_kikibouba_v1_k4_%j.err     # Error file
...
echo ""
echo "=== Starting training with K=4 on KikiBouba v1 ==="
echo "Config: configs/kikibouba_v1.yaml"
echo "GPUs: $NPROC"
echo ""
...
torchrun --standalone --nproc_per_node="${NPROC}" \
    train.py \
    --config configs/kikibouba_v1.yaml
