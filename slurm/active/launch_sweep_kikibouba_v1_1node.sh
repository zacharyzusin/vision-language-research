#!/bin/bash
# Script to initialize wandb sweep for KikiBouba v1 (single-node) and submit Slurm jobs to run sweep agents
# Usage: ./launch_sweep_kikibouba_v1_1node.sh [num_agents]
...
echo "=== Initializing W&B Sweep for KikiBouba v1 (Single-Node) ==="
echo "Sweep config: sweep_k_values_kikibouba_v1_1node.yaml"
echo "Number of agents: $NUM_AGENTS"
echo ""

# Initialize the sweep and get the sweep ID
SWEEP_ID=$(wandb sweep sweep_k_values_kikibouba_v1_1node.yaml 2>&1 | grep -oP 'wandb agent [^\s]+' | awk '{print $3}' || echo "")
...
echo ""
echo "View logs:"
echo "  tail -f logs/sweep_kb1_1gpu_*.out"
