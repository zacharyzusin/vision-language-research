...
#SBATCH --output=logs/sweep_kb1_1node_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_kb1_1node_%j.err     # Error file
...
echo "Single-node DDP configuration (KikiBouba v1):"
...
# The agent will call run_sweep_training_kikibouba_v1_1node.sh for each sweep run, which launches torchrun
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_kikibouba_v1_1node.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_kikibouba_v1_1node.sh

# Run wandb agent - it will execute run_sweep_training_kikibouba_v1_1node.sh for each sweep run
