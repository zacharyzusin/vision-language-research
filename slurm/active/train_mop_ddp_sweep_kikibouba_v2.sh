...
#SBATCH --output=logs/sweep_kb2_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_kb2_%j.err     # Error file
...
# The agent will call run_sweep_training_kikibouba_v2.sh for each sweep run, which launches torchrun
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_kikibouba_v2.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_kikibouba_v2.sh

# Run wandb agent - it will execute run_sweep_training_kikibouba_v2.sh for each sweep run
