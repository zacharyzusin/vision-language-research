...
#SBATCH --job-name=inat_bulrush_sweep # Job name
...
#SBATCH --output=logs/sweep_bulrush_1gpu_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_bulrush_1gpu_%j.err     # Error file
...
echo "Running wandb sweep agent for iNaturalist Bulrush subset sweep ID: $SWEEP_ID"

# Single-GPU setup
echo "Single-GPU configuration (iNaturalist Bulrush subset):"
...
# The agent will call run_sweep_training_subset_bulrush.sh for each sweep run
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_subset_bulrush.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_subset_bulrush.sh

# Run wandb agent - it will execute run_sweep_training_subset_bulrush.sh for each sweep run
