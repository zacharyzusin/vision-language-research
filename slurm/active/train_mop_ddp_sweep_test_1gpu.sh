...
#SBATCH --job-name=sweep_test_1gpu   # Job name
...
#SBATCH --gres=gpu:A6000:1            # Request 1x A6000 GPU (for quick test)
#SBATCH --exclude=ins082              # ins082 has a bad GPU device handle
#SBATCH --output=logs/sweep_test_1gpu_%j.out     # Output file (%j = job ID)
#SBATCH --error=logs/sweep_test_1gpu_%j.err     # Error file
...
# The agent will call run_sweep_training_test_1gpu.sh for each sweep run
echo "Launching wandb agent (sweep ID: $SWEEP_ID)..."
echo "Agent will run training jobs using: ./run_sweep_training_test_1gpu.sh"

# Make sure the wrapper script is executable
chmod +x run_sweep_training_test_1gpu.sh

# Run wandb agent - it will execute run_sweep_training_test_1gpu.sh for each sweep run
# The wrapper script handles single-GPU setup
# By default, agent runs until sweep is complete (2 K values)
